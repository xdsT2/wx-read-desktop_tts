 ...
 *
 * v2 修复：
 * - TextChunker 用 matchAll 保留分隔符切分，startIndex 精确
 * - PlaybackStore 不再存全文，改用 textHash + textLength 验证
 * - 增加 speechSynthesis 可用性检测与错误提示
 * - 增加 onError 回调，UI 展示错误条
 */
...
(function () {
  ...
  WebTTSEngine.prototype.speak = function (text) {
    ...
    if (zhVoices.length === 0 && !this._voice) {
      console.warn('[TTS] 未检测到中文语音，将使用默认语音');
  ...
  var PlaybackStore = {
    ...
    clearAll: function () {
      var keys = [];
      for (var i = 0; i < localStorage.length; i++) { var k = localStorage.key(i); if (k && k.indexOf(PlaybackStore.PREFIX) === 0) keys.push(k); }
  ...
  TTSPlayer.prototype.checkAvailability = function () {
    ...
    if (!avail.supported) { this.showError(avail.message); return; }
    if (!avail.hasChineseVoice && this.settings.showFallbackHint) { this.showError(avail.message); }
  };
